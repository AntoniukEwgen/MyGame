...
namespace Vampire
{
    ...
    public class Monster : IDamageable, ISpatialHashGridClient
    {
        ...
        [SerializeField] protected float despawnTimeout = 10f;  // Time in seconds before the monster despawns if not within camera bounds
        ...
        protected Coroutine despawnCoroutine = null;
        ...
        public virtual void Setup(int monsterIndex, Vector2 position, MonsterBlueprint monsterBlueprint, float hpBuff = 0)
        {
            ...
            monsterHitbox.size = monsterSpriteRenderer.bounds.size / 3;
            ...
            // Start the despawn check coroutine
            if (despawnCoroutine != null)
            {
                StopCoroutine(despawnCoroutine);
            }
            despawnCoroutine = StartCoroutine(CheckDespawn());
        }
