    ...
    public class FixedDirectionStabAbility : StabAbility
    {
        protected override IEnumerator Stab()
        {
            ...
            while (t < stabTime)
            {
                ...
                if (attackAngle > 90 || attackAngle < -90) // Если угол атаки больше 90 или меньше -90 градусов
                {
                    weaponSpriteRenderer.flipY = true; // Поворачиваем по оси Y
                }
                else
                {
                    weaponSpriteRenderer.flipY = false; // Не поворачиваем по оси Y
                }
